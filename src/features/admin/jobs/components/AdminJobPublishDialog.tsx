...
import { addDaysToDateInput, getJobsSpotTodayDateInput, jobsSpotEndOfDayToIso, toJobsSpotDateInput } from "@/lib/jobsSpotDateTime";
...
function defaultDeadline(job: AdminJobDetails) {
    ...
    if (current && current > new Date()) return toJobsSpotDateInput(current);
...
export default function AdminJobPublishDialog({ job, open, onOpenChange }: { job: AdminJobDetails; open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    async function handlePublish() {
        ...
        try {
            const response = await mutation.mutateAsync({ applicationDeadline: deadline ? jobsSpotEndOfDayToIso(deadline) : undefined });
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                ...
                <div className="space-y-3">
                    ...
                    <Input id="admin-publish-deadline" type="date" value={deadline} min={getJobsSpotTodayDateInput()} onChange={(event) => setDeadline(event.target.value)} />
