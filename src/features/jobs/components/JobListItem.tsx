...
import { ArrowRight, Banknote, Building2, Clock3, MapPin } from "lucide-react";
...
function formatSalary(job: PublicJob) {
    ...
    if (salaryMin !== null && salaryMax !== null) {
        salaryText = `${formatter.format(salaryMin)} – ${formatter.format(salaryMax)}`;
    } else if (salaryMin !== null) {
        salaryText = `From ${formatter.format(salaryMin)}`;
    } else {
        salaryText = `Up to ${formatter.format(salaryMax ?? 0)}`;
    }

    if (job.salaryPeriod) {
        return `${salaryText} / ${formatLabel(job.salaryPeriod).toLowerCase()}`;
    }
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-5 flex flex-wrap items-center gap-x-6 gap-y-3 text-lg text-slate-600">
                            ...
                            <div className="flex items-center gap-2 lg:hidden">
                                <Banknote size={19} className="shrink-0 text-slate-400" />
